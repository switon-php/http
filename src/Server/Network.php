...
/**
 * IP matching and local-address helper.
 *
 * Road-signs:
 * - contains() supports exact, wildcard, range, and CIDR matches
 * - local() picks a usable local address for server defaults
 *
 * @see \Switon\Http\RequestInterface::ip()
 */
class Network
{
    /**
     * @param string|list<string> $haystack
     */
    ...
    public static function local(): string
    {
        if (function_exists('swoole_get_local_ip')) {
            ...
            $ips = exec('hostname --all-ip-addresses 2>/dev/null');
