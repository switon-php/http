...
namespace Switon\Http\Tests\Unit\Request;
...
class FileTest extends TestCase
{
    ...
    protected function createFileData(
        ...
        return [
            ...
            'name' => $name,
            'type' => 'text/plain',
            'tmp_name' => $tmpName ?? $this->testFile,
            'error' => $error,
            'size' => $size
        ];
    }
    ...
    public function testGetExtensionReturnsFinalSegmentForMultiDotFilename(): void
    {
        $file = $this->container->make(File::class, ['file' => $this->createFileData('archive.tar.gz')]);
        $this->assertSame('gz', $file->getExtension());
    }

    public function testGetExtensionReturnsEmptyWhenFilenameHasNoDot(): void
    {
        $file = $this->container->make(File::class, ['file' => $this->createFileData('Makefile')]);
        $this->assertSame('', $file->getExtension());
    }
    ...
    public function testMoveToThrowsWhenDestinationHasNoAllowedExtension(): void
    {
        $this->pathAlias->set('@uploads', $this->tempDir . '/uploads');
        mkdir($this->tempDir . '/uploads', 0755, true);

        $this->expectException(FileException::class);
        $this->file->moveTo('@uploads/readme', 'pdf,doc');
    }
    ...
    public function testDeleteIsNoOpWhenTemporaryPathDoesNotExist(): void
    {
        $missing = $this->tempDir . '/not_there_' . uniqid('', true);
        $this->assertFileDoesNotExist($missing);

        $file = $this->container->make(File::class, [
            'file' => $this->createFileData('still.txt', UPLOAD_ERR_OK, 12, $missing),
        ]);
        $file->delete();
    ...
    public function testDeleteSwallowsFilesystemDeleteFailure(): void
    {
        $original = $this->container->get(FilesystemInterface::class);
        try {
            $filesystem = $this->createMock(FilesystemInterface::class);
            $filesystem->method('exists')->willReturn(true);
            $filesystem->method('delete')->willReturnCallback(static function (): void {
                \Switon\Core\Exception\RuntimeException::raise('unlink denied');
            });
            $this->container->replace(FilesystemInterface::class, $filesystem);

            $file = $this->container->make(File::class, ['file' => $this->createFileData()]);
            $file->delete();
            $this->assertTrue(true);
        } finally {
            $this->container->replace(FilesystemInterface::class, $original);
        }
    }
